...
//                                      LogDebug(sprintf($this->format . "%-16s", "VIDEO", $packetTS, $this->prevVideoTS, $packetSize, $this->pVideoTagPos));
    ...
    var saveToDisk = function(callback) {
        ...
        for(let i = 0; len = F4F.HDS.getFragsArray().length, i < len; i++) {
            let curItem = F4F.HDS.getFragsArray()[i];
            if(curItem) {
                //updateHTML.showMergingArrayPartsProgress(i, len - 1);
           ...
           // let media = F4F.HDS.getMediaArray()[0][1];
            ...
            function downloadSliceHDS(i) {
                ...
                function handleErr(i) {
                    ...
                    if (retry_counts[i] > RETRY_LIMIT)
                        return false;
                    else {
                        setTimeout(downloadSliceHDS.bind(null, i), 2000);
                        retry_counts[i] = retry_counts[i] ? retry_counts[i]+1 : 1;
                        return true;
                    }
                }
                xhr[i] = new XMLHttpRequest();
                xhr[i].open("GET", fragUrl, true);
                xhr[i].onreadystatechange = function() {
                    if (xhr[i].readyState == 4) {
                        if (xhr[i].status == 200) {
                            console.log(`Done, ts index: ${i}, ts url ${fragUrl}`);
                            ...
                            console.log(`Failed, ts index: ${i}, ts url ${fragUrl}`);
